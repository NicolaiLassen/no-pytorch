...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.proj = nn.Linear(50, 64, 1)
        self.petrov_galerkin = GalerkinTransformer(dim=64, qkv_pos=RoPE(16), dim_head=16, depth=1, heads=4)
        self.fourier = FNO2d(in_channels=64, out_channels=3, freq_dim=20, fourier_modes=8, depth=3, grid=False)
        
    def forward(self, x, f):
        _, t, _, w, h = x.shape

        f = repeat(f, "b c -> b t c w h", t=t, w=w, h=h)
        
        x = torch.cat([f, x], dim=2)
        
        x = rearrange(x, 'b t c w h -> b (t c) w h').permute(0, 2, 3, 1)
        ...
        x = self.fourier(x).permute(0, 1, 2, 3).unsqueeze(1)
...
class MagDatasetLite(Dataset):
    ...
    def _initialize(self):
        seq, field = read_h5_dataset(self.data_path, self.block_size, self.batch_size, self.stride, self.n_data)
